...
but global opposing-state support is stronger and the local count adds no
reliable partial association ($p=.200$). The contribution is therefore a
controlled composition test and an evidence-bounded diagnosis of its failure,
not a new architecture or causal shortcut claim.
\end{abstract}
...
We contribute a data-audited observed-composition protocol, a controlled
target-present/absent comparison, and a six-architecture stress test. We also
show why a plausible local label diagnostic must be interpreted against simpler
support controls. Figure \ref{fig:protocol} shows the naturally observed diagnosis
spaces and targets retained by the support rule.
...
$G_c=\sum_{i\in\mathcal T}\mathbb 1[y_{i,c}\ne y_c^*]$. Target frequency,
cardinality, and total one-bit neighborhood density are constants across criteria
inside a diagnosis and cancel under within-diagnosis ranking. LOP is an exposure
association, not causal influence or deployment confidence.
...
\subsection{RQ2: local opposition does not beat marginal support}
...
label-neighborhood association from being mistaken for unique or causal evidence.
...
Label-space analysis predicts vulnerable criteria, but simple marginal support
explains more than local opposition. The result is a reproducible composition
test with both a strong controlled effect and a bounded mechanism claim.
