...
Transformer, and state-space backbones, removing target-diagnosis examples lowers
exact diagnosis match from 26.96\% to 1.52\% (target bootstrap gap 25.44 points,
95\% CI $[17.31,33.32]$; two-sided $p=.00049$); bit accuracy falls 19.08 points.
...
training-group pool. Thus the conditions have identical size, validation and
test rows, initialization, class weights, and zero group overlap; only the target
diagnosis is present versus absent. This test uses four generic temporal
...
100,000-sample target bootstrap intervals and two-sided probabilities.

\section{Results}
\subsection{RQ1: matched removal exposes diagnosis dependence}
Under matched test recordings, target-present exact match is 26.96\%, versus
1.52\% when the target is absent from training: a 25.44-point gap (95\% CI
$[17.31,33.32]$; two-sided Wilcoxon $p=.00049$). The gap is positive for every
architecture (20.32 TCN, 27.08 BiGRU, 22.87 Transformer, and 31.48 SSM points).
Bit accuracy also falls 19.08 points (CI $[13.62,26.10]$; $p=.00049$). RQ1 is
therefore supported without comparing different evaluation distributions.
...
size, class weights, initialization, validation, and test rows are matched.}
\label{fig:matched}
\end{figure}
...
identical criterion marginals; shared class weights reduce but cannot remove that
limitation. Manual criterion--anatomy mappings affect FACT,
...
training reduces exact match by 25.44 points across four architecture families.
